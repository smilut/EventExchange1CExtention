...
Процедура exch_ОткрытьФайлВыгрузки(КомпонентыОбмена) Экспорт
	...
	ЗаписьСообщения.НомерСообщения = ДанныеПолучателя.НомерОтправленного;
...
Функция exch_УстановитьЗначенияОтборов(Набор, ОбъектВыгрузки)
	Для Каждого Реквизит Из ОбъектВыгрузки Цикл
...
Функция exch_НаборЗаписейДляВыгрузки(ОбъектВыгрузки)
	Если СтрНайти(ОбъектВыгрузки.ТипОбъекта, "РегистрСведений.") = 1 Тогда
		Набор = РегистрыСведений[Метаданные.НайтиПоПолномуИмени(ОбъектВыгрузки.ТипОбъекта).Имя].СоздатьНаборЗаписей();		
	ИначеЕсли СтрНайти(ОбъектВыгрузки.ТипОбъекта, "РегистрНакопления.") = 1 Тогда
		Набор = РегистрыНакопления[Метаданные.НайтиПоПолномуИмени(ОбъектВыгрузки.ТипОбъекта).Имя].СоздатьНаборЗаписей();		
	ИначеЕсли СтрНайти(ОбъектВыгрузки.ТипОбъекта,"РегистрБухгалтерии.") = 1 Тогда
		Набор = РегистрыБухгалтерии[Метаданные.НайтиПоПолномуИмени(ОбъектВыгрузки.ТипОбъекта).Имя].СоздатьНаборЗаписей();		
	Иначе
		Возврат Неопределено;
	КонецЕсли;
	
	Возврат exch_УстановитьЗначенияОтборов(Набор, ОбъектВыгрузки);
КонецФункции
...
Процедура exch_ВыполнитьВыгрузкуЗарегистрированныхДанных(КомпонентыОбмена, НомерСообщения)
	...
	ОбъектВыгрузки = КомпонентыОбмена.exch_ОбъектВыгрузки;
	Если exch_ОбменДаннымиПовтИсп.ЭтоМетаданныеСсылочногоТипаПовтИсп(ОбъектВыгрузки.ТипОбъекта) Тогда
		фильтр = ОбъектВыгрузки.Ссылка;
	Иначе
		фильтр = exch_НаборЗаписейДляВыгрузки(ОбъектВыгрузки);
	КонецЕсли;
	
	Если фильтр = Неопределено Тогда
		//если фильтр не определен, то
		//выобъект выгрузки передан с ошибкой
