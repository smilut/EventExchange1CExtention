//в обработчики подписок на событие ПередЗаписью, которые выполняют проверку
//правил регистрации и заполняют узлы получатели
//добавляем назначение обработчика подписки на событие ПриЗаписи, который
//выполняет анализ узлов получателей, формирует и отправляет сообщения обмена
...
Процедура НазначитьОбработчикОтправкиДанныхПоСобытию(Источник, ИмяСобытия, ИмяОбработчика)
	...
	Если получателиСобытия.Количество() > 0 Тогда		
		Источник.ДополнительныеСвойства.Вставить(exch_ОбменДаннымиНастройки.ИмяСвойстваУзлыПолучателиСобытия(), получателиСобытия);
		ОбменДаннымиСобытия.СократитьПолучателей(Источник, получателиСобытия);
